...
def ridge(X, y, num_lambdas=100, k=5):
    ...
    return([errors, fold_means, fold_means[optimal_tune_index], B_list, tuning_params, tune_star])
...
def elastic_net(X_train, y_train, num_lambdas=50, k=5):
    ...
    l2 = np.geomspace(1e-6, 50000, num_lambdas)
    ...
    for i in fold_index:
        ...
        y_train = np.vstack([y[j] for j in np.delete(fold_index, i)])
        X_train_std, X_test_std = utils.standardize(X_train, X_test)
        B = [lr.elastic_net(X_train_std, y_train, l[0], l[1]) for l in tuning_params]
        error = [utils.get_error(B[j], np.hstack((X_test_std, y_test))) for j in np.arange(num_lambdas)]
        B_list = B_list + [B]
        errors = errors + [error]
    fold_means = np.mean(np.array(errors),0)

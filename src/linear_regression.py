...
def lasso(X, y, l, tol=1e-6):
    m, n = np.shape(X)
    B_star = np.ones((n))
    B_s = np.zeros((n))
    while np.any(np.absolute(B_s - B_star) > tol):
        for j in np.arange(len(B_s)):
            k = np.where(B_s!= 0)[0]
            update = (1/m)*((np.dot(np.transpose(X[:,j]),y) - 
                             np.dot(np.dot(np.transpose(X[:,j]), X[:,k]),B_s[k]))) + B_s[j]
            B_star[j] = np.sign(update) * max(abs(update) - l, 0)
        B_s = B_star
    return np.append(np.mean(y), B_star)
...
#     if l_path.size == 0:
#         if l == 0:
#             l_ = np.append(np.geomspace(l_max,l+1e-6,99),0)
#         else: 
...
#             B_s = B_star
#             for j in np.arange(len(B_s)):
#                 r = y - np.reshape(np.dot(X, B_s), (m, 1))
#                 update = (1/m) * np.dot(X[:,j], r) + B_s[j]
#                 # update = (1/m)*(np.dot(X[:,j],y) - \
#                 #                 sum([sum(X[:,j] * X[:, z]) * B_s[z] for z in k]) + \
#                 #                 np.dot(X[:,j],X[:,j]) * B_s[j])
#                 B_star[j] = utils.soft_thresholding_operator(update, l_[i])
#             tot_coeff[i,:] = B_star
#             if np.all(np.absolute(B_s - B_star) < tol):
#                 break
